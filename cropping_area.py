...
class CroppingArea:
    ...
    def create_handles(self):
        ...
        self.canvas.create_rectangle(up_coords, fill='white', tags=('upper_handle', 'cropping_area'))
        self.canvas.create_rectangle(down_coords, fill='white', tags=('lower_handle', 'cropping_area'))
        self.canvas.create_rectangle(left_coords, fill='white', tags=('left_handle', 'cropping_area'))
        self.canvas.create_rectangle(right_coords, fill='white', tags=('right_handle', 'cropping_area'))

        self.canvas.create_polygon(
            upper_left_coords, fill='white', tags=('upper_left_handle', 'cropping_area'), outline='black'
        )
        self.canvas.create_polygon(
            lower_left_coords, fill='white', tags=('lower_left_handle', 'cropping_area'), outline='black'
        )
        self.canvas.create_polygon(
            upper_right_coords, fill='white', tags=('upper_right_handle', 'cropping_area'), outline='black'
        )
        self.canvas.create_polygon(
            lower_right_coords, fill='white', tags=('lower_right_handle', 'cropping_area'), outline='black'
        )
    ...
    def create_dashed_lines(self):
        ...
        self.canvas.create_line(upper_left_line, dash=(10,), fill='white', width=l_w, tags=('upper_left_line', 'cropping_area'))
        self.canvas.create_line(upper_right_line, dash=(10,), fill='white', width=l_w, tags=('upper_right_line', 'cropping_area'))
        self.canvas.create_line(left_upper_line, dash=(10,), fill='white', width=l_w, tags=('left_upper_line', 'cropping_area'))
        self.canvas.create_line(left_lower_line, dash=(10,), fill='white', width=l_w, tags=('left_lower_line', 'cropping_area'))
        self.canvas.create_line(lower_left_line, dash=(10,), fill='white', width=l_w, tags=('lower_left_line', 'cropping_area'))
        self.canvas.create_line(lower_right_line, dash=(10,), fill='white', width=l_w, tags=('lower_right_line', 'cropping_area'))
        self.canvas.create_line(right_lower_line, dash=(10,), fill='white', width=l_w, tags=('right_lower_line', 'cropping_area'))
        self.canvas.create_line(right_upper_line, dash=(10,), fill='white', width=l_w, tags=('right_upper_line', 'cropping_area'))
    ...
    def upper_left_handle_move(self, event):
        ...
        points = self.multiple_adding_difference(
                [
                    (self.canvas.coords('upper_left_handle'), 'upper_left_handle', 'diff_xy', diff_x, diff_y),
                    (self.canvas.coords('left_handle'), 'left_handle', 'diff_xy', diff_x, diff_y),
                    (self.canvas.coords('upper_handle'), 'upper_handle', 'diff_xy', diff_x, diff_y),
                    (self.canvas.coords('lower_left_handle'), 'lower_left_handle', 'diff_x', diff_x),
                    (self.canvas.coords('upper_right_handle'), 'upper_right_handle', 'diff_y', diff_y),
                ]
            )
        self.multiple_coords_changing(points)
    ...
    def upper_right_handle_move(self, event):
        ...
        points = self.multiple_adding_difference(
            [
                (self.canvas.coords('upper_right_handle'), 'upper_right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('right_handle'), 'right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('upper_handle'), 'upper_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_right_handle'), 'lower_right_handle', 'diff_x', diff_x),
                (self.canvas.coords('upper_left_handle'), 'upper_left_handle', 'diff_y', diff_y),
            ]
        )
        self.multiple_coords_changing(points)
    ...
    def lower_left_handle_move(self, event):
        ...
        points = self.multiple_adding_difference(
            [
                (self.canvas.coords('lower_left_handle'), 'lower_left_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('left_handle'), 'left_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_handle'), 'lower_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('upper_left_handle'), 'upper_left_handle', 'diff_x', diff_x),
                (self.canvas.coords('lower_right_handle'), 'lower_right_handle', 'diff_y', diff_y),
            ]
        )
        self.multiple_coords_changing(points)
    ...
    def lower_right_handle_move(self, event):
        ...
        points = self.multiple_adding_difference(
            [
                (self.canvas.coords('lower_right_handle'), 'lower_right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('right_handle'), 'right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_handle'), 'lower_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('upper_right_handle'), 'upper_right_handle', 'diff_x', diff_x),
                (self.canvas.coords('lower_left_handle'), 'lower_left_handle', 'diff_y', diff_y),
            ]
        )
        self.multiple_coords_changing(points)
    ...
    def move_hor(self, event, tag):
        ...
        if tag == 'left_handle':
            ...
            if x0 + diff_x + min_h > rx or x0 + min_h - h_w == rx and edge_dist_l < self.start_edge_dist_l:
                diff_x = self.canvas.coords('right_handle')[2] - x0 - min_h
        else:
            ...
            if x1 + diff_x - min_h < lx or x1 - min_h == lx and edge_dist_r > self.start_edge_dist_r:
                diff_x = self.canvas.coords('left_handle')[0] - x1 + min_h
    ...
    def move_vert(self, event, tag):
        ...
        if tag == 'upper_handle':
            ...
            if y0 + diff_y + min_h > ly or y0 + min_h - h_w == ly and edge_dist_u < self.start_edge_dist_u:
                diff_y = self.canvas.coords('lower_handle')[3] - y0 - min_h
        else:
            ...
            if y1 + diff_y - min_h < uy or y1 - min_h == uy and edge_dist_d > self.start_edge_dist_d:
                diff_y = self.canvas.coords('upper_handle')[1] - y1 + min_h
    ...
    def area_move(self, event):
        ...
        points = self.multiple_adding_difference(
            [
                ...
                (self.canvas.coords('upper_right_handle'), 'upper_right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_right_handle'), 'lower_right_handle', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('upper_left_line'), 'upper_left_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('upper_right_line'), 'upper_right_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('left_upper_line'), 'left_upper_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('left_lower_line'), 'left_lower_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_left_line'), 'lower_left_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('lower_right_line'), 'lower_right_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('right_lower_line'), 'right_lower_line', 'diff_xy', diff_x, diff_y),
                (self.canvas.coords('right_upper_line'), 'right_upper_line', 'diff_xy', diff_x, diff_y),
            ]
        )
        self.multiple_coords_changing(points)
